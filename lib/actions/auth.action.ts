...
export async function loginWithCredentials(params: Pick<AuthCredentials, 'email' | 'password'>): Promise<ActionResponse> {
    const validResult = await action({ params, schema: LoginSchema }) // Validate the input parameters against the schema

    if (validResult instanceof Error) return handleError(validResult) as ErrorResponse; // If validation fails, handle the error and return an ErrorResponse

    const { email, password } = validResult.params!; // Destructure the valid result (not used here, but can be used for further processing)

    try {
        const existingUser = await User.findOne({ email }); // Check if a user with the provided email already exists
        if (!existingUser) throw new NotFoundError("User"); // If a user exists, throw an error
        ...
        if (!existingAccount) throw new NotFoundError("Account"); // If a account exists, throw an error
        ...
        if (!isValidPassword) throw new Error("Invalid credentials"); // If password is invalid, throw an error
